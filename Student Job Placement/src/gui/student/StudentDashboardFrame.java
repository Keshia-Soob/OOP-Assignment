...
public class StudentDashboardFrame extends BaseFrame {
    ...
    protected JComponent buildContent() {
        ...
        int acceptedOffer = JobOfferDAO.hasAcceptedOffer(userId) ? 1 : 0;

        int selected = selectedOnCampus + selectedOffCampus + acceptedOffer;
    ...
    private Color statusColor(String status) {
        switch (status) {
            case "Shortlisted":
                return new Color(230, 160, 20);
            case "Selected":
                return new Color(34, 139, 34);
            case "Rejected":
                return new Color(190, 50, 50);
            default:
                return new Color(52, 102, 160);
        }
    }
